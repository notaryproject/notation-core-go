...
func RegisterEnvelopeType(mediaType string, newFunc NewEnvelopeFunc, parseFunc ParseEnvelopeFunc) error {
	...
	if !isValidEnvelopeType(mediaType) {
		return fmt.Errorf("invalid mediatype %s", mediaType)
	}
...
func NewEnvelope(mediaType string) (Envelope, error) {
	if !isValidEnvelopeType(mediaType) {
		return nil, fmt.Errorf("invalid envelope type: %s", mediaType)
	}
...
func isValidEnvelopeType(mediaType string) bool {
	return mediaType == COSEMediaTypeEnvelope || mediaType == JWSMediaTypeEnvelope
}
